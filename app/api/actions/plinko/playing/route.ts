    ...
    export const POST = async (req: Request) => {
        try {
            ...
            const payload = {
                type: "playing",
                icon: new URL("/gif.gif", new URL(req.url).origin).toString(),
                duration: 5000, // Duration in milliseconds
                next: {
                    type: "post",
                    href: "/api/actions/rps/result", // Next phase route
                },
                claimButton: {
                    text: "Claim Reward",
                    href: "/api/actions/rps/result", // The route when the button is clicked
                }
            };
